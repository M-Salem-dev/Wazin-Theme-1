...
$(".box-btn-Profile-icon .nav-link").click(function () {
  // functionاعمل خصائص ال btn لم اضغط ع
  // console.log("ccccccccccc");
    ...
    $("html, body").animate(
      ...
      10
    ); // 1000 هو وقت التمرير بالمللي ثانية (1 ثانية)
...
$(".menu-item").click(function(){	
	$(".sub-menu-dropdown").slideToggle(1000);	// لو مخفي بظهر و لو ظهر بيختفي ع شكل ستاره 
});
...
$(document).ready(function () {
  ...
  if ($(".box-item-Favorites-nav").length > 0) {
    ...
    $(".box-Empty-Favorites").css("display", "none");
  }
  ...
  $(".btn-trash").click(function () {
    ...
    if (itemContainer.length) {
      ...
      if ($(".box-item-Favorites-nav").length === 0) {
        // If there are no cart, hide the container
        $(".box-Empty-Favorites").css("display", "block");
      }
    }
  });
});
